...
interface Run {
  num: string
  phase: string
  outcome: 'ongoing' | 'extinction' | 'stable' | 'collapse'
  date: string
  pop: string
  map: string
  body: string[]
...
const RUNS: Run[] = [
  ...
  },
  {
    ...
    outcome: 'ongoing',
    date: 'Spring Year 0',
    pop: '30 spawned — 10 alive at Day 4',
    map: '1000×1000 Azgaar grid',
    body: [
      'First run on the 1000×1000 real-geography grid with biome-gated entity spawning. Group B spawned at (843–850, 425) — a resource desert — and was fully extinct by t3800 of Day 0. Group C mostly collapsed by Day 2. Group A landed in the spawn haven and stabilised at 92% average satiety by Day 4.',
      's_a1 and s_a2 have reached 100% trust with 138 interactions. Reproduction conditions not yet met — shelter is the suspected blocker. s_a5 reached 28yr with 5 social relationships. No forage failure events observed for the surviving group.',
    ],
    findings: [
      'Spawn anchor algorithm based on grid resource channel is unreliable — does not reflect actual entity density post-spawn',
      'Groups B and C died in entity-sparse zones despite adequate grid R values',
      'Core survival loop confirmed working for Group A — commit windows and drain rates are correct',
      'Socialise interaction count inflated — cooldown helps but 138 interactions in 4 days still high',
      '191 apex on the map — too dense, inflates fear baseline constantly',
    ],
  },
  {
    num: 'Run 04',
...
const OUTCOME_BG: Record<string, string> = {
  ongoing:   'rgba(34,197,94,0.12)',
  stable:    'rgba(34,197,94,0.12)',
  extinction:'rgba(225,29,72,0.12)',
  collapse:  'rgba(251,113,133,0.12)',
}
    ...
    <>
      ...
      <section style={{ padding: 'clamp(3.5rem, 10vw, 6.5rem) 0' }}>
        <div style={{ width: 'min(calc(100% - 2 * var(--gutter)), var(--max-wide))', marginInline: 'auto' }}>
          <Reveal>
            <p style={{ fontSize: '0.6875rem', fontWeight: 600, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--crimson-bright)', marginBottom: '0.75rem' }}>Phase 2–4 — Survival loop</p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            {RUNS.map((run, i) => (
              <Reveal key={run.num} delay={i * 0.07}>
                <div style={{ background: 'var(--bg-raised)', border: '1px solid var(--border-soft)', borderRadius: '14px', overflow: 'hidden' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem 1.25rem', borderBottom: '1px solid var(--border-soft)', flexWrap: 'wrap', gap: '0.5rem' }}>
                    ...
                    <span style={{ fontSize: '0.6875rem', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: OUTCOME_COLOR[run.outcome], background: OUTCOME_BG[run.outcome], padding: '0.2rem 0.65rem', borderRadius: '100px' }}>
                      {run.outcome}
                    </span>
                  </div>
        ...
        <div style={{ width: 'min(calc(100% - 2 * var(--gutter)), var(--max-read))', marginInline: 'auto' }}>
          ...
          <Reveal delay={0.1}>
            <div style={{ color: 'var(--text-muted)', maxWidth: '52ch', marginBottom: '2rem' }}>
              ...
              <p style={{ margin: '0 0 1rem' }}>Observations are recorded by day: entity counts, survival rates, notable events. Memory tab snapshots are taken on notable agents every 2–3 days.</p>
